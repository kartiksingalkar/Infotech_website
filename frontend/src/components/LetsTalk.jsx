...
const LetsTalk = (props) => {
    ...
    return (
        <>
            <Validations />
            <Popup ref={ref}  trigger={<div className={props.name==='Lets Talk'? 'lets-talk-container' : 'home-carousel-button'}><a>{props.name}</a></div>} position={props.name==='Lets Talk'? 'center center' : 'right center'}
            ...
            {!showpopup && <Popup open={secondOpen} position="center center" onClose={closeModal1}
                closeOnDocumentClick={false}>
                <div className='popup_form_box'>
                    <h3>OTP Successfully Sent On <span>{data.mobile}</span></h3>
